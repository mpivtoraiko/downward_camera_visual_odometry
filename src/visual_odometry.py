...
def compute_vo(img1_raw, img2_raw, K):
   ...
   if len(good_new) != num_feat:
      print(f"Warning: number of new features ({len(good_new)}) does not match number of old features ({num_feat})")
      return None

   if len(good_new) < 5 or len(good_old) < 5:
      print(f"Error: not enough features to estimate: {len(good_old)}, {len(good_new)}")
      return None
